...
function buildEventShareFromPredictions(
  ...
  const rowsAll: EventShareRow[] = rows.map((r) => {
    ...
    let who = (r.athlete_name || '').trim();
    if (!who || who === '-') who = ({ WINNER: 'WINNER', PODIUM: 'PODIUM PICK', HIGHEST_SCORE: 'TOP SCORE' } as Record<string, string>)[r.market_type] || (disc ? `${disc} PICK` : 'PICK');
...
const TournamentDetail = () => {
  ...
  const handleConfirmPodiumPrediction = async (stakeAmount: number) => {
    ...
    try {
      ...
      // Atomically deduct from wallet (prevents race conditions)
